...
eventalert.usage = "\nfhc eventalert create <appid> alertName categories=eventCategories severities=eventSeverities events=eventNames emails=emailAddresses"
  + "\nfhc eventalert clone <alertId>"
  + "\nfhc eventalert delete <alertId>"
  + "\nfhc eventalert list <appId>"
  + "\nfhc eventalert read <alertId>"
  + "\nfhc eventalert read <appid> alertName"   // NOTE: fhc uses --dev and --live and environment config to specify dev or live"
...
function eventalert (args, cb) {
  ...
  if (args.length < 2) return cb("Invalid arguments:" + eventalert.usage);
