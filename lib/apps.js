...
var apps = module.exports = {
  ...
  create: function(options, appTitle, repo, branch, cb) {
    ...
    api.doAppCall(options, "create", payload, "Error creating new app: ", function(err, data) {
  ...
  update: function(appId, name, value, cb) {
  ...
  setKnownProperty: function(options, appId, key, value, cb){
    apps.read(options, appId, function(err, data){
      if (err) return cb(err, null);
      ...
      api.doAppCall(options, "update", payload, function (error, data) {
        if (error) {
          cb(err, null);
        }
        else {
          if (data.status != 'ok') {
            return cb(remoteData.messsage, data);
          }
          return cb(null, data);
        }
      });
