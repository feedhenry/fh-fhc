...
function doInitLocal (appId, cb) {  
  ...
  log.silly('Getting app container from url: ' + fhreq.getFeedHenryUrl() + ", domain: " + fhc.target, 'get container');
  common.readApp(appId, function (err, data) { // check valid appid
    if (err) return cb(err);
