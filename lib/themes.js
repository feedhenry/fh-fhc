...
function doUpdate(theme, cb) {
  fhreq.POST(fhreq.getFeedHenryUrl(), "api/v2/themes/theme", theme, function (err, remoteData, raw, res) {
    if (err) return cb(err);
    if (res.statusCode !== 200) return cb('Unexpected response code: ' + res.statusCode + ' - ' + raw);
...
function doGetTheme(themeId, cb) {
  fhreq.GET(fhreq.getFeedHenryUrl(), "api/v2/themes/theme/" + themeId, function (err, remoteData, raw, res) {
    if (err) return cb(err);
    if (res.statusCode !== 200) return cb('Unexpected response code: ' + res.statusCode + ' - ' + raw);
