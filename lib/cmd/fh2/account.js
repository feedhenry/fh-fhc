...
account.desc = "Manage your FeedHenry Account Resources";
account.usage = "\n fh account upload-resource <destination> <file-path> <resource-type> <config>"
  ...
  + "\n or"
  + "\n fh account upload-resources-batch <destination> <resource-directory>"
  ...
  + "\n    |--sigtool.db"
  + "\n or"
  + "\n fh account generate-resource <destination> <resource-type> <password>"
...
function account(argv, cb){
  ...
  switch(action){
    ...
    case "generate-resource": return generateResource(args[0], args[1], args[2], cb);
...
function uploadResourcesBatch(destination, filepath, cb){
  fs.exists(filepath, function(exists){
    ...
    fs.stat(filepath, function(err, fstat){
      ...
      if(fstat.isFile()){
        return cb(filepath + " is a file. Please use 'fh account upload-resource' instead.");
      }
...
function generateResource(dest, type, password, cb){
  var genFunc = function(res_type, destination, pass, callback){
    var gen_url = "box/srv/1.1/dev/account/res/generate";
    common.doApiCall(fhreq.getFeedHenryUrl(), gen_url, {type: res_type, dest: destination, password: pass}, "Error generating account resource:", function(err, res){
      log.silly("Response of generating resource " + res_type + " for " + destination + " : " + JSON.stringify(res));
      if(res.result === "ok"){
  ...
  if(dest.toLowerCase() === "android"){
    var list_url = "box/srv/1.1/dev/account/res/list";
    common.doApiCall(fhreq.getFeedHenryUrl(), list_url, {"dest":"android"}, "Error listing account resources:", function(err, res){
      var dist_cert_found = false;
      if(res.certificates && res.certificates.length > 0){
        for(var i=0;i<res.certificates.length;i++){
          if(res.certificates[i].type === "distribution"){
      ...
      if(dist_cert_found){
        prompt("Warning: Found an existing distribution certificate for Android in your account. It will be overwritten and can not be recovered. Are you sure you want to continue ? \n (Y/N):", "", false, function(er, val){
          if(val.toLowerCase() === "y" || val.toLowerCase() === "yes"){
            genFunc("certificate", dest, password, cb);
          } else {
    ...
    return cb("Destination " + dest + " is not supported at the moment. ");
  }
}
