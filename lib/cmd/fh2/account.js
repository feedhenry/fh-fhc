        ...
        _.each(fileTypes, function (cfgs, destination) {
          _.each(cfgs, function (types, cfg) {
            _.each(types, function (paths, filetype) {
              if (paths.length > 1) {
                return log.warn("Found more than 1 resources for destination " + destination +
                  ", config : " + cfg +
                  ", type : " + filetype + ". Ignored.");
              }
              var path = paths[0];
...
function generateResource(dest, type, password, cb) {
  var genFunc = function (resType, destination, pass) {
    ...
    common.doApiCall(fhreq.getFeedHenryUrl(), genUrl, {
      ...
      log.silly("Response of generating resource " + resType + " for " + destination + " : " + JSON.stringify(res));
      if (res.result === "ok") {
  ...
  if (dest.toLowerCase() === "android") {
    ...
    common.doApiCall(fhreq.getFeedHenryUrl(), listUrl, {"dest": "android"}, "Error listing account resources:", function (err, res) {
