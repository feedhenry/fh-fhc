...
act.desc = "Perform a FeedHenry act request";
act.usage = "\nfh app act <app-id> <server-function> <params> --env=<environment>";
...
function doMillicoreAct(appId, widgId, funct, data, cb) {
  ...
  fhreq.POST(fhreq.getFeedHenryUrl(), "box/srv/1.1/act/" + fhc.curTarget + "/" + widgId + "/" + funct + "/" + appId, data, function (err, remoteData, raw, response) {
    if (err) {
...
act.completion = function (opts, cb) {
  ...
  // only complete for 3'rd param, i.e. 'fh act <tab>'
  if (argv.length === 2) {
    common.getAppIds(cb);
  }
};
