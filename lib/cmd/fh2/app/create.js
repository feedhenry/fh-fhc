...
create.desc = "Creates a FeedHenry app";
create.usage = "fh app create <app-title> [<git-repo> <git-branch>]";
create.usage_ngui = "fh app create <project-id> <app-title> [<template-id>]" +
                  "\nfh app create <project-id> <app-title> <template-type> <git-repo> [<git-branch>]";
...
function doCreate(appTitle, repo, branch, cb) {
  ...
  common.doApiCall(fhreq.getFeedHenryUrl(), "box/srv/1.1/ide/" + fhc.curTarget + "/app/create", params, "Error creating new app: ", function(err, data) {
    ...
    if (data.isScmApp && data.isScmPrivate) {
      ...
      create.message += "fh git pull " + data.guid;
    }
