...
configuration.desc = "Manage client app configuration properties";
configuration.usage = "\nfh configuration [list] <app-id>"
           + "\nfh configuration [list] <app-id> <destination>"
           + "\nfh configuration set <app-id> <destination> <key> <value>"
...
function configuration (argv, cb) {
  ...
  if (args.length === 1) {
    return list(fhc.appId(args[0]), 'all', cb);
  }

  var action = args[0];
  if (action === 'list') {
    if (args.length !== 2 && args.length !== 3) return usage(cb);
    list(fhc.appId(args[1]), args[2], cb);
  } else if (action === 'set'){
    if (args.length !== 5) return usage(cb);
    set(fhc.appId(args[1]), args[2], args[3], args[4], cb);
  } else if (args.length === 2) {
    return list(fhc.appId(args[0]), args[1], cb);
  } else {
    return usage(cb);
  }
}
