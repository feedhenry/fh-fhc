...
deployTargets.desc = "Operations on Cloud Deploy Targets";
deployTargets.usage = "\nfh deploy-target create name=<the name of the deploy target> platform=<cloudfoundry/stackato/appfog/ironfoundry> conf_url=<deploy url> conf_username=<user for the deploy> conf_password=<user password for the deploy> --env=<environment> [conf_infrastructure=<infrastructure providers>] ..."
                    + "\nfh deploy-target update id=<id of the deploy target> name=<the name of the deploy target> platform=<cloudfoundry/stackato> conf_url=<deploy url> conf_username=<user for the deploy> conf_password=<user password for the deploy> --env=<environment> ... "
                    + "\nfh deploy-target read <id of the deployment target>"
                    + "\nfh deploy-target list"
                    + "\nfh deploy-target delete <id of the deployment target>"
                    + "\nfh deploy-target listforapp <appid> [<platform>] --env=<environment>"
                    + "\nfh deploy-target getapptarget <appid> --env=<environment>"
                    + "\nfh deploy-target setapptarget <appid> <id of the deploy target> --env=<environment>"
...
function deployTargets(argv, cb){
  ...
  if(action === "list"){
    ...
    if(args.length < 6) return cb("Invalid arguments for 'create':" + deployTargets.usage);
    ...
    if(args.length < 2) return cb("Invalid arguments for 'getapptarget':" + deployTargets.usage);
