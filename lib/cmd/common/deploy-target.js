...
function deployTargets(argv, cb){
  ...
  if(action === "list"){
    ...
    if(args.length < 6) {
      return cb("Invalid arguments for 'create':" + deployTargets.usage);
    }
    return createDeployTarget(argv, cb);
  } else if(action === "update"){
    if(args.length < 2) {
      return cb("Invalid arguments for 'update':" + deployTargets.usage);
    }
    return updateDeployTarget(argv, cb);
  } else if(action === "read"){
    if(args.length < 2) {
      return cb("Invalid arguments for 'read':" + deployTargets.usage);
    }
    return readDeployTarget(argv, cb);
  } else if(action === "delete"){
    if(args.length < 2) {
      return cb("Invalid arguments for 'delete':" + deployTargets.usage);
    }
    return deleteDeployTarget(argv, cb);
  } else if(action === "listforapp"){
    if(args.length < 2) {
      return cb("Invalid arguments for 'listforapp':" + deployTargets.usage);
    }
    return listAvailableDeployTargets(argv, cb);
  } else if(action === "getapptarget"){
    if(args.length < 2) {
      return cb("Invalid arguments for 'getapptarget':" + deployTargets.usage);
    }
    return getDeployTargetForApp(argv, cb);
  } else if(action === "setapptarget"){
    if(args.length < 3) {
      return cb("Invalid arguments for 'setapptarget':" + deployTargets.usage);
    }
