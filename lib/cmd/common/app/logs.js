...
logs.desc = "Cloud application log Files";
logs.usage =  "\nfh app logs [get] <app-id> [log-name] --env=<environment>"
            + "\nfh app logs tail <app-id> [last-N-lines] [offset-from] [log-name] --env=<environment>"
            + "\nfh app logs list <app-id> --env=<environment>"
            + "\nfh app logs delete <app-id> <log-name> --env=<environment>";
            + "\n"
...
function listLogs (appId, target, cb) {
  ...
  common.doApiCall(fhreq.getFeedHenryUrl(), "box/srv/1.1/ide/" + fhc.curTarget + "/app/logs", payload,"Error getting logs: ", function(err, data){
...
function deleteLog (appId, logName, target, cb) {
  ...
  common.doApiCall(fhreq.getFeedHenryUrl(), "box/srv/1.1/ide/" + fhc.curTarget + "/app/logs", payload,"Error deleting log: ", function(err, data){
    if(err) return cb(err);
...
function logChunk(appName, last, offset, logName, cb) {
  ...
  common.doApiCall(fhreq.getFeedHenryUrl(), "box/srv/1.1/ide/" + fhc.curTarget + "/app/logchunk", payload, "", function(err, data) {
    return cb(err, data);
  });
}
