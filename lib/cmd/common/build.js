...
build.usage_ngui = "\nfhc build project=<project-id> app=<app-id> cloud_app=<cloud-app-id> tag=<tag> destination=<destination> version=<version> config=<config> keypass=<private-key-password> certpass=<certificate-password> download=<true|false> bundleId=<credential bundle id> cordova_version=<cordova version>  branch=<branch name> commit=<commit hash>"
  + i18n._("\nwhere <destination> is one of: android, iphone, ipad, ios(for universal binary), blackberry, windowsphone7, windowsphone (windows phone 8)")
  + i18n._("\nwhere <version> is specific to the destination (e.g. Android version 4.0)")
  + i18n._("\nwhere <config> is either 'debug' (default), 'distribution', or 'release'")
