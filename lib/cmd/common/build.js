...
build.usage = "\nfh build app=<app-id> destination=<destination> version=<version> config=<config> keypass=<private-key-password> certpass=<certificate-password> download=<true|false> provisioning=<path-to-provisioning-profile> cordova_version=<cordova version>"
...
build.usage_ngui = "\nfh build project=<project-id> app=<app-id> cloud_app=<cloud-app-id> tag=<tag> destination=<destination> version=<version> config=<config> keypass=<private-key-password> certpass=<certificate-password> download=<true|false> provisioning=<path-to-provisioning-profile> cordova_version=<cordova version>"
  + "\nwhere <destination> is one of: andriod, iphone, ipad, ios(for universal binary), blackberry, windowsphone7, windowsphone (windows phone 8)"
  + "\nwhere <version> is specific to the destination (e.g. Android version 4.0)"
  + "\nwhere <config> is either 'debug' (default), 'distribution', or 'release'"
  + "\nwhere <provisioning> is the path to the provisioning profile"
...
function build(argv, cb) {
  ...
  try{
    ...
    validateArgs(argObj);
    if (isNGUI) validateArgsNgui(argObj);
  } catch (x) {
...
function validateArgsNgui(args) {
  if (args.project === undefined) throw new Error("Missing 'project' parameter");
  if (args.cloud_app === undefined) throw new Error("Missing 'cloud_app' parameter");
  if (args.tag === undefined) throw new Error("Missing 'tag' parameter");
};
