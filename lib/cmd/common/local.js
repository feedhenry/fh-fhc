...
local.usage = ["",
               " fh local <cloud_mBaaS_instance_id>",
               " This command (or the equivalent 'fh initlocal <appID>') must be run at least",
               ...
               "   openClient : indicates whether the client URL (typically http://127.0.0.1:8000) should be opened in the default browser when fh local starts. Default value is false.",
               " Examples : ",
               "   fh local fh3=true client=./my-project-client-app cloud=./my-project-cloud-app/cloud",
               "   fh local clientHost=http://192.168.28.34 clientPort=9000 cloudHost=http://192.168.28.34 cloudPort=9001",
               "   fh local startCloud=false",
               "   fh local packages=app",
               "   fh local debug=true debugBrk=true redisHost=192.168.28.33 redisPort=10001 decoupled=true",
...
function local(argv, cb) {
  ...
  if (!fs.existsSync(fhcLocalDir)) {
    if (!argObj.appid) {
      return cb("Error : 'fh initlocal <appId>' or 'fh local <appId>' never called, see the usage for an explanation.\n\nUsage: " + local.usage);
    }
  }
  async.series([
