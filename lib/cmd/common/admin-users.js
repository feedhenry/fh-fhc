...
users.desc = "Administer FeedHenry Users";
users.usage = "\nfh admin-users list"
             +"\nfh admin-users create username=<username> [password=<password>] [email=<email>] [name=<name>] [roles=<roles>] [authpolicies=<authpolicies>] [invite=<invitation>] [storeItemGroups=<groups>]"
             +"\nfh admin-users delete <username>"
             +"\nfh admin-users read <username>"
             +"\nfh admin-users update username=<username> [password=<password>] [email=<email>] [name=<name>] [roles=<roles>] [authpolicies=<authpolicies>] [inviation=<invitation>] [enabled=<enabled>] [storeItemGroups=<groups>]"
             +"\nfh admin-users enable <username>"
             +"\nfh admin-users disable <username>"
             +"\nfh admin-users changeroles <username> <roles>"
             +"\nfh admin-users changeauthpolicies <username> <authpolicies>"
             +"\nfh admin-users listdevices <username>"
             +"\nfh admin-users listinstallapps <username>"
             +"\nfh admin-users import <path-to-csv-file> [invite] [<roles> <authpolicies>]";
...
function users (argv, cb) {
  ...
  if (action === 'list') {
    ...
    if (args.length !== 3) return cb("Invalid arguments for 'changeauthpolicies':" + users.usage);
