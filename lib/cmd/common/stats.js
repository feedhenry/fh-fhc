...
stats.desc = "Feedhenry cloud application statistics, counters & timers";
stats.usage = "\nfh stats <app-id> <stats-type> <num-results> --env=<environment>";
...
function readAppStats(appId, statsType, numResults, target, cb) {
  ...
  common.doApiCall(fhreq.getFeedHenryUrl(), "box/srv/1.1/ide/" + fhc.curTarget + "/app/stats", payload, "Error reading app statistics: ", cb);
}
...
stats.completion = function (opts, cb) {
  ...
  // only complete for 3'rd param, i.e. 'fh stats <tab>'
  if (argv.length === 2) {
    common.getAppIds(cb);
  }
};
