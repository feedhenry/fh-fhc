...
files.desc = "Manage application files";
files.usage = "\nfh files [list] <app-id>"
  + "\nfh files read <file-id>"
  + "\nfh files update <app-id> <file-id> <file>"
  + "\nfh files create <app-id> <path> <name> <fileType>"
  + "\nfh files delete <app-id> <file-id> <path> <name> <fileType>";
...
function list (appId, cb) {
  ...
  millicore.widgForAppId(appId, function (err, widgId) {
    ...
    common.doApiCall(fhreq.getFeedHenryUrl(), "box/srv/1.1/ide/" + fhc.curTarget + "/file/list", payload, "Error listing file: ", function(err, data){
      var filez = {};
...
function read (fileId, cb) {
  ...
  common.doApiCall(fhreq.getFeedHenryUrl(), "box/srv/1.1/ide/" + fhc.curTarget + "/file/read", payload, "Error reading file: ", cb);
}
...
function create (appId, path, name, fileType, cb){
  ...
  millicore.widgForAppId(appId, function (err, widgId) {
    ...
    log.silly(widgId, "widgId");
      ...
      common.doApiCall(fhreq.getFeedHenryUrl(),"box/srv/1.1/ide/" + fhc.curTarget + "/file/create", payload, "Error creating file: ", cb);
  });
}
...
function deleteFile (appId, fileId, path, name, type, cb){
  ...
  millicore.widgForAppId(appId, function (err, widgId) {
    ...
    log.silly(widgId, "widgId");
      ...
      common.doApiCall(fhreq.getFeedHenryUrl(),"box/srv/1.1/ide/" + fhc.curTarget + "/file/delete", payload, "Error deleting file: ", cb);
  });
}
...
files.completion = function (opts, cb) {
  ...
  // only complete for the 4'th param, i.e. fh files list <tab>
  if (argv.length === 3) {
    var action = argv[2];
    switch (action) {
      case "list":
