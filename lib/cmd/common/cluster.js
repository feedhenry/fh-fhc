...
function read(cb){
    ...
    common.doGetApiCall(fhreq.getFeedHenryUrl(), url, "Error reading Cluster Props: ", function(err, cluster) {
...
function userCreate(args, cb){
    ...
    if(! user) return cb("username required");
    if(! pass) return cb("pass required");

    var payload = {"username":user,"pass":pass};

    if(email){
        payload.email = email;
    }
    var url = "/box/api/clusters/users";
    common.doApiCall(fhreq.getFeedHenryUrl(), url, payload, "failed to create cluster user",function(err, user){
...
function userList(args,cb){
    ...
    common.doGetApiCall(fhreq.getFeedHenryUrl(), url,  "failed to create cluster user",function(err, users){
        if (err) return cb(err);
        return cb(err, users);
    });
}
...
function userRead(args,cb){
    ...
    common.doGetApiCall(fhreq.getFeedHenryUrl(), url,  "failed to read cluster user",function(err, users){
        if (err) return cb(err);
        return cb(err, users);
    });
}
...
function userUpdate(args,cb){
    ...
    if(! payload)return cb("not updating anything? ");
