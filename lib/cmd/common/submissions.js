...
submissions.usage = "fh submissions [list]\n" +
              "fh submissions list app=<app-id> form=<form-id>\n" +
              "fh submissions get <submission-id>\n" +
              "fh submissions get <submission-id> <filename>.pdf\n" +
              "fh submissions delete <submission-id>\n" +
              "fh submissions getfile <filegroup-id> <outputfilename>\n" +
              "fh submissions submitdata <submission-file.json>\n" +
              "fh submissions submitfile <submission-id> <field-id> <file-id> <file>\n" +
              "fh submissions complete <submission-id>\n" +
              "fh submissions status <submission-id>\n" +
              "fh submissions export file=<zip-file> [app=<app-id>] [form=<form-id>] [fieldHeader=<<name || fieldCode>>]\n" +
              "fh submissions template <app-id> <form-id>";
...
function getAppFormIds(args, cb) {
  var appId, formId, file, fieldHeader;
  ...
  for (var i=0; i<args.length; i++) {
    ...
    }else if (arg.indexOf('fieldHeader=') === 0){
      fieldHeader = arg.replace('fieldHeader=', '');
    }else {
      return cb('Unknown argument: ' + arg + '\n' + submissions.usage);
    }
  }
  process.nextTick(function() {
    return cb(null, appId, formId, file, fieldHeader);
  });
}
...
function doList(args, cb) {
  ...
  getAppFormIds(args, function(err, appId, formId) {
    ...
    fhreq.POST(fhreq.getFeedHenryUrl(), "api/v2/forms/submission", payload, function (err, remoteData, raw, res) {
      ...
      remoteData.submissions = _.map(remoteData.submissions, function(submission){
        return sanitizeSubmissionData(submission);
      });
...
/**
 * Function for pruning any unneeded data from submissions.
 * @param submissionJSON
 */
function sanitizeSubmissionData(submissionJSON){
  submissionJSON.formFields = _.map(submissionJSON.formFields, function(field){
    var type = field.fieldId.type;
    var fieldValues = field.fieldValues;

    if(type === "photo" || type === "signature" || type === "file"){
      fieldValues = _.map(fieldValues, function(fieldValue){
        var url = fieldValue.url || fieldValue.downloadUrl || "/api/v2/forms/submission/file/" + fieldValue.groupId;

        //Avoiding getting // in the url
        if(url.indexOf("/") === 0){
          url = url.substr(1, url.length);
        }

        url = fhreq.getFeedHenryUrl() + url;
  ...
  //Removing data from submissions that is used in studio.
  submissionJSON.formSubmittedAgainst.pages = _.map(submissionJSON.formSubmittedAgainst.pages, function(page){
...
function doGetSubmission(submissionId, cb) {
  fhreq.GET(fhreq.getFeedHenryUrl(), "api/v2/forms/submission/" + submissionId, function (err, remoteData, raw, res) {
    ...
    return cb(err, sanitizeSubmissionData(remoteData));
  });
}
...
function doExport(args, cb) {
  var payload = {};
  getAppFormIds(args, function(err, appId, formId, file, fieldHeader) {
    if (err) return cb(err);
    if (!file) return cb("No file specified, use 'file=<zip-file>, e.g. 'file=/tmp/foo.zip'\n" + submissions.usage);

    if (appId) payload.appId = appId;
    if (formId) payload.formId = formId;
    if (fieldHeader) payload.fieldHeader = fieldHeader;
