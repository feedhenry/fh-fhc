...
git.desc = "Git operations on a FeedHenry App which doesn't have it's own repo"; // TODO Is this still valid in FH3?
git.usage = "\nfh git [props] <app-id>"
  + "\nfh git set <app-id> <git-prop-name> <git-prop-value>"
  + "\nfh git pull <app-id>";
...
function set (appId, k, v, cb) {
  common.readApp(appId, function(err, cfg){
    ...
    fhreq.POST(fhreq.getFeedHenryUrl(), "box/srv/1.1/ide/" + fhc.curTarget + "/app/update", payload,  function (err, remoteData, raw, response) {
      if (err) {
...
function getGitProps(appId, cb) {
  ...
  var payload = {payload:{guid: appId}};
  common.doApiCall(fhreq.getFeedHenryUrl(), "box/srv/1.1/ide/" + fhc.curTarget + "/app/read", payload, "Error reading Git properties: ", function(err, data){
    if(err) return cb(err);
