...
module.exports = {
  ...
  'examples': [
    ...
    {
      cmd: "fhc app create --project=1a2b3c --title=My New App --type=cloud_nodejs --repo=git:///some.com/repo.git --branch=my-branch",
  ...
  'preCmd': function(argv, cb) {
    ...
    if (argv.repo) {
      ...
      if ( argv.branch ) {
        argv.branch =  "refs/heads/" + argv.branch;
      } else {
        argv.branch = "refs/heads/master";
      }

      params.template = {
        repoUrl: argv.repo,
        repoBranch: argv.branch,
        type: argv.type,
        imported: true
      };
      ...
      fhc.templates.apps({id:templateId, json:true}, function(err, template) {
        if (err) {
          return cb(err);
        }
