...
module.exports = {
  ...
  'customCmd' : function(params, cb) {
    ...
    return common.doApiCall(fhreq.getFeedHenryUrl(), url, playload, i18n._("Error creating env var: "), function(err, data) {
      ...
      if (!params.json) {
        return cb(null, util.format(i18n._("Environment variables for the app '%s' with the name '%s' and value '%s' into the environment '%s' created successfully."),params.app,params.name, params.value, environment));
      } else {
        return cb(null, data);
      }
    });
