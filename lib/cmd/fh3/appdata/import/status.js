 ...
 * - step: current import job step
 * - totalSteps: total import steps
 */
function readProgress(params, cb) {
  var url = 'api/v2/appdata/' + params.envId + '/' + params.appId + '/import/data/' + params.jobId;
...
module.exports = {
  ...
  'examples': [{
    cmd: [
      'fhc addpata import status --appId=5ujx2eifvzaudq43nw4nmvcu --envId=live --jobId=5731da1dd3e2b283203c4054'
    ].join(''),
    desc: i18n._('Poll for job status')
  }],
  ...
  'customCmd': function (params, cb) {
    ...
    // Need to use `console#log` here until we introduce some other logging or output
    // framework to fhc
    /* eslint disable-next-line no-console */
    ...
    function getProgressBar(job) {
      if (bar === null && job.totalSteps) {
        // When translating the following string, pay attention to the templates (:bar, :percent, etc.):
        // they must be put inside the translated string as they are (they can be moved, but must retain the same names)
