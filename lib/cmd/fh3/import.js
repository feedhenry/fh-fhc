...
function importZipFile(params, cb) {
  if (!fs.existsSync(params.zipFile)) {
    return cb(i18n._("File doesn't exist: ") + params.zipFile);
  }

  log.silly(params.zipFile, "app import");
  ...
  var formData = {
    ...
    templateZipFile: fs.createReadStream(params.zipFile)
  };
  ...
  request.post({url: url, headers:headers, proxy: fhc.config.get("proxy"), formData: formData}, function(err, res, data) {
    ...
    if (res.statusCode !== 201) {
      return cb(i18n._('Error importing zip file: ') + res.body);
    }
    if (!params.json) {
      return cb(null, i18n._('Import via zip file completed successfully'));
    }
    return cb(null, data);
  });
}
