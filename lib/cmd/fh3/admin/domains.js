...
function createDomain(args, cb) {
  ...
  common.doApiCall(fhreq.getFeedHenryUrl(), url, payload, "failed to create cluster user", cb);
}

function checkDomain(args, cb) {
  var domainToCheck = args[1];
  var url = '/box/api/domains/check?domain=' + domainToCheck;
  common.doGetApiCall(fhreq.getFeedHenryUrl(), url, "Error checking domain availability: ", cb);
}
