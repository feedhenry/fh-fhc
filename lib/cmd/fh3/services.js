...
services.desc = i18n._("Manage FeedHenry Services");
...
function services(argv, cb) {
  ...
  if ("list" === action) {
    ...
    return unknown(util.format(i18n._("Invalid service action %s"), action), cb);
  }
}
...
function listServices(args, cb) {
  common.listServices(function (err, projs) {
    ...
    if (ini.get('table') === true) {
      services.table = common.createTableForServices(projs);
    }
...
function readService(serviceId, cb) {
  common.doGetApiCall(fhreq.getFeedHenryUrl(), "box/api/connectors/" + serviceId, i18n._("Error reading Service: "), cb);
}
...
function deleteService(args, cb) {
  ...
  common.doDeleteApiCall(fhreq.getFeedHenryUrl(), endpoint, {},  i18n._("Error deleting service: "), function (err, data) {
    if (err) return cb(err);
    return cb(err, data);
  });
}
