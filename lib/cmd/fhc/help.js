...
module.exports = { 
  ...
  'examples' : [
    { cmd : 'fh help', desc : 'Retrieves help for all of FHC'},
    { cmd : 'fh help app', desc : 'Retrieves help with the app command'}
  ],
  'demand' : [],
  'alias' : {},
  globalFlags : {
  ...
  fhcUsage : function(cb){
    ...
    msg.push("\nAdd --help to any command for quick help, or use fh help <command>.");
    return cb(null, msg.join('\n'));
  },
  commandGroupUsage : function(cmds, cmdFn, cb){
    var msg = [];
    msg.push('fh ' + cmds.join(' ') + ' <action>');
  ...
  singleCommandDocPage : function(cmds, cmdFn, cb){
    ...
    section_path = path.join('doc', command._path.replace(/\.js$/, '.md'));
    ...
    return fs.stat(section_path, function (err, out) {
      ...
      child = spawn('less', [section_path], { customFds: [ 0, 1, 2 ] }); // TODO: Worth noting customFds is deprecated
