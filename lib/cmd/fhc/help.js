...
module.exports = {
  ...
  singleCommandDocPage: function(cmds, cmdFn, cb) {
    ...
    var i18n_ll_cc_section_path = path.join('doc', 'languages', ll_cc, command._path.replace(/\.js$/, '.md'));
    var i18n_ll_section_path = path.join('doc', 'languages', ll, command._path.replace(/\.js$/, '.md'));

    var self = this;
    var help_func = function(fn, err) {
      // TODO Use latest nodejs api. customFds is deprecated
      var child = spawn('less', [fn], {customFds: [0, 1, 2]});
      if (child.stderr) {
    ...
    return fs.stat(i18n_ll_cc_section_path, function(err) {
      if (err && err.code === 'ENOENT') {
        return fs.stat(i18n_ll_section_path, function(err) {
          if (err && err.code === 'ENOENT') {
            return fs.stat(section_path, function(err) {
              ...
              return help_func(section_path, err);
            });
          } else {
            return help_func(i18n_ll_section_path, err);
          }
        });
      } else {
        return help_func(i18n_ll_cc_section_path, err);
      }
    });
  },
