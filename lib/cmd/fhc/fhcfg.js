...
config.desc = "Manage the FHC Configuration File";
config.usage =   "fh fhcfg set <key> <value>"
             + "\nfh fhcfg get <key>"
             + "\nfh fhcfg delete <key>"
             + "\nfh fhcfg list"
             + "\nfh fhcfg edit";
...
function edit (cb) {
  ...
  ini.save(function (er) {
    ...
    fs.readFile(f, "utf8", function (er, data) {
      ...
      data = [ ";;;;"
             , "; fh "+(ini.get("global") ? "globalconfig" : "userconfig")+" file"
             , "; this is a simple ini-formatted file"
             , "; lines that start with semi-colons are comments, and removed."
             , "; read `fh help config` for help on the various options"
             , ";;;;"
             , ""
             , data
             ].concat( [ ";;;;"
...
function list (cb) {
  ...
  if (!long) {
    msg += "; node install prefix = " + process.installPrefix + "\n"
         ...
         + "; 'fh config ls -l' to show all defaults.\n";
  }
...
config.completion = function (opts, cb) {
  ...
  if (argv[1] !== "fhcconfig") argv.unshift("fhcconfig");
  if (argv.length === 2) {
