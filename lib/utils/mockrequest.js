...
var mockHosts = {
  "hosts": {
    "development-dyno": "mockdomain-dev",
    "development-name": "mockdomain-mockguid1234567890123456-dev",
    "development-url": "https://mockdomain-mockguid1234567890123456-dev.feedhenry.com",
    "live-dyno": "ngui-demo-live",
    "live-name": "ngui-demo-fhkaj4obkj97vzaogemgxpyd-live",
    "live-url": "https://mockdomain-mockguid1234567890123456-live.feedhenry.com"
  },
  "status": "ok"
}
...
function mockRequest (fhurl, method, where, what, etag, nofollow, cb_) {
  ...
  if (where.match(new RegExp('box/srv/1.1/ide/.*?/app/list'))) {
    data = mockAppList;
  }else if (where.match(new RegExp('box/srv/1.1/ide/.*?/app/read'))) {
    data = mockAppRead;
  }else if (where.match(new RegExp('box/srv/1.1/ide/.*?/app/delete'))) {
    data = mockAppDelete;
  }else if (where.match(new RegExp('box/srv/1.1/ide/.*?/app/create'))) {
    data = mockAppCreate;
  }else if (where.match(new RegExp('box/srv/1.1/ide/.*?/app/logs'))) {
    data = mockAppLogs;
  }else if (where.match(new RegExp('box/srv/1.1/tst/version'))){
    data = mockVersion;
  }else if (where.match(new RegExp('box/srv/1.1/act/.*?/c0TPJzF6ztq0WjezxwPEC5W8/getCloudData/0123'))) {
    data = mockAct;
  }else if (where.match(new RegExp('box/srv/1.1/act/.*?/c0TPJzF6ztq0WjezxwPEC5W8/getLiveCloudData/0123'))) {
    data = mockLiveAct;
  }else if (where.match(new RegExp('box/srv/1.1/ide/.*?/app/read'))) {
    data = mockWidgetRead;
  }else if (where.match(new RegExp('box/srv/1.1/ide/.*?/file/list'))) {
    data = mockFileList;
  }else if (where.match(new RegExp('box/srv/1.1/ide/.*?/file/read'))) {
    data = mockFileRead;
  }else if (where.match(new RegExp('box/srv/1.1/ide/.*?/app/read'))) {
    data = mockWidgetRead;
  }else if (where.match(new RegExp('box/srv/1.1/ide/.*?/file/update'))) {
    data = mockFileUpdate;
  }else if (where.match(new RegExp('box/srv/1.1/ide/.*?/file/delete'))) {
    data = mockFileDelete;
  }else if (where.match(new RegExp('box/srv/1.1/ide/.*?/file/create'))) {
    data = mockFileCreate;
  }else if (where.match(new RegExp('/box/srv/1.1/act/sys/auth/login'))) {
    data = mockLogin;
  }else   if (where.match(new RegExp('sys/info/ping'))) {
    data = mockMsgPing;
  }else if (where.match(new RegExp('sys/info/version'))) {
    data = mockMsgVersion;
  }else if (where.match(new RegExp('sys/info/stats'))){
    data = mockMsgStats;
  }else if (where.match(new RegExp('msg'))){
    data = mockMsgTopics;
  }else if (where.match(new RegExp('msg/profile'))){
    data = mockMsgTopic;
  }else if (where.match(new RegExp('msg/profile'+'?'+'123'))){
    data = mockMsgTopicQuery;
  }else  if (where.match(new RegExp('box/srv/1.1/tst/randomint'))) {
    data = mockPing;
  }else if (where.match(new RegExp('box/srv/1.1/sys/admin/domain/props?d=.*?'))){
    data = mockPropsDomain;
  }else if (where.match(new RegExp('box/srv/1.1/ide/.*?/app/list'))){
    data = mockPropsApp;
  }else if (where.match(new RegExp('box/srv/1.1/ide/.*?/app/setconfig'))){
    data = mockPropsSet;
  }else if (where.match(new RegExp('box/srv/1.1/ide/.*?/user/read'))){
    data = mockUser;
  }else if (where.match(new RegExp('box/srv/1.1/dat/log/read(.*?)'))) {
    data = mockCacheKeys;
  }else if(where.match(new RegExp("box/srv/1.1/ide/.*?/api/list"))){
    data = mockKeyList;
  }else if(where.match(new RegExp("box/srv/1.1/ide/.*?/api/create"))){
    data = mockKeyCreate;
  }else if(where.match(new RegExp("box/srv/1.1/ide/.*?/api/delete"))){
    data = mockKeyRevoked;
  }else if(where.match(new RegExp("box/srv/1.1/ide/.*?/api/update"))) {
    data = mockKeyUpdated;
  }else if (where.match(new RegExp("box/srv/1.1/ide/.*?/app/hosts"))) {
    data = mockHosts;
  }
  else {
  ...
  return cb_(err, data, JSON.stringify(data), response);
};
