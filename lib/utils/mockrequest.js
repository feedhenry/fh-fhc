...
function mockRequest (fhurl, method, where, what, etag, nofollow, cb_) {
  ...
  if (where.match(new RegExp('box/srv/1.1/ide/.*?/app/list'))) {
    ...
    data = mockKeyUpdated;
  } else if(where.match(new RegExp("api/v2/forms/submission")) && method === "POST"){
    data = mockSubmissionList;
  } else if(where.match(new RegExp("api/v2/forms/submission/5449109299e98c45145f8267")) && method === "GET"){
    data = mockSubmissionGet;
  }
  else {
