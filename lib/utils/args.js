...
function _normaliseFlagsAndArs(argsIntoCommand, cmd) {
  ...
  const normalisedArgs = _.clone(argsIntoCommand);

  _.each(cmd.alias, function(alias, key) {
    if (key.match(/[0-9]/)) {
      var position = parseInt(key, 10),
        arg = argsIntoCommand[position];
      var hasArgumentAtPosition = !isNaN(position) && typeof arg !== 'undefined' && !arg.match(/^--/) && arg.indexOf('=') === -1;
      if (hasArgumentAtPosition) {
        var idx = normalisedArgs.indexOf(arg);
        if (idx > -1) {
          normalisedArgs[idx] = '--' + alias + '=' + arg;
        }
      }
    }
  });
