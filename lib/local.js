...
LocalServer.prototype.handleRequest = function(request, response) {
  ...
  for(var i = 0, il = handlers.length; i < il && !handled; i++) {
    ...
    if(handled = handler.canHandle(request)) {
...
RequestHandler.prototype.handleRequest = function(request, response) {
  response.writeError(404, "No handler supplied");
};
...
LocalActHandler.prototype.init = function() {
  ...
  try {
    ...
    if(!Object.keys(env).length) {
      throw Error("Looks like you didn't export any functions. Is this a rhino app?")
    }
    if(env) {
      this.env = env;
    ...
    if(this.env) {
      ...
      console.log("Failed to reload environment, using current version");
      console.error(e);
    }
    else {
      throw e;
