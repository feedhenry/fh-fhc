...
function local(args, cb) {
  ...
  var server = http.createServer(function(request, response) {
    if (request.url === '/box/srv/1.1/app/init') {
      response.end(JSON.stringify({"domain":"apps","firstTime":false,"hosts":{"debugCloudType":"fh","debugCloudUrl":"http://127.0.0.1:8888","releaseCloudType":"fh","releaseCloudUrl":"http://127.0.0.1:8888"},"status":"ok"}));
    }
    else if(actHandler.canHandle(request)) {
...
LocalActHandler.prototype.handleRequest = function(request, response) {
  ...
  request.on("end", function() {
    ...
    // include headers set by client
    params = params || {};
    params._headers = request.headers;
    log.silly(params, "params");
...
StaticHandler.prototype.buildIndex = function(file, requestQuery) {
  var html = new Element("html"),
    ...
    sdkInitScript = new Element("script"),
    sdkScript = new Element("script"),
    sdkConfigScript = new Element("script"),
    sdkStyle = new Element("style");
  ...
  if(requestQuery.device !== undefined && requestQuery.version !== undefined) {
    ...
    sdkScript.setAttribute("src", "https://testing-lon3.feedhenry.com/static/pec/script/studio/23-scripts.js");
  }

  sdkConfigScript.append('var fh_destination_code = \'studio\';' +
    'var fh_app_version = \'0\';' +
    'var fh_config = {host: "http://127.0.0.1:8888",appid: "localguidplaceholder",appkey: "localappkeyplaceholder",mode: "dev"};');
  sdkInitScript.append("$fh.ready({}, function(){$fh.init(fh_config);});");

  sdkStyle.append('html,body{margin:0;padding:0}');

  //head.append(contentType, apiScript, sdkInitScript);
  head.append(contentType, sdkConfigScript, sdkScript, sdkInitScript, sdkStyle);
  body.append(file);
