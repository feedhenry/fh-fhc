...
function local(args, cb) {
  ...
  var appid;
  try {
    appid = args.splice(0,1).toString(); //splice will return an array, but we want the string
  ...
  async.series([
    ...
    function(callback) {
      initlocal.getEnvVars(appid.toString(), function(err, envVars){
    ...
    function(callback) {
      ...
      console.log("appEnvironment = " + JSON.stringify(appEnvironment));
