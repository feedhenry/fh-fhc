...
fhc.load = function(conf, cb) {
  ...
  async.series({
    ...
    _.deepExtend(fhc, commands.fhc, commands['fh3'], commands.internal);
...
fhc.applyCommandFunction = function(processArgv, cb) {
  ...
  if (cmdFn.demand) {
    if (cmdFn.deprecation) {
      console.warn(colors.yellow(cmdFn.deprecation +"\n"));
    }
    try {
      argv = argUtils.validate(yargs, cmdFn);
    } catch (err) {
      if (cmdFn.deprecation) {
        console.warn(colors.yellow(cmdFn.deprecation +"\n"));
      }
