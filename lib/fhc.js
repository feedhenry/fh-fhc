...
fhc.applyCommandFunction = function(cmd, argv, cb){
  ...
  */
  process.nextTick(function(){
    // If our command implements a yargs based definition, perform the validation here
    if (typeof cmdFn === 'object'){
      // Path for new post-yargified commands
      genericCommand(argv, cmdFn, cb);
    }else{
      // Path for old pre-yargified commands
      cmdFn(argv, cb);  
    }  
  });
