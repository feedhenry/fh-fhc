...
templates.usage_ngui = "\nfhc templates apps [<template-id>]"
    ...
    + "\nfhc templates forms [<template-id>]"
    + "\nfhc templates themes [<template-id>]";
...
var formHeaders = ['Id', 'Name', 'Description', 'Last Updated'];
var formFields = ['_id', 'name', 'description', 'lastUpdated'];
var themeHeaders = ['Id', 'Name', 'Last Updated'];
var themeFields = ['_id', 'name', 'lastUpdated'];

// main templates entry point
function templates (args, cb) {
  ngui([], function(err, isNGUI) {
    if (err) return cb(err);
    if (isNGUI === false) return oldTemplates(args, cb);

    if (args.length < 1 || args.length > 2) return cb(templates.usage_ngui);

    var action = args[0];
    var templateId = args[1];
    if (action === 'apps') {
      if (templateId) return getTemplate('apps', templateId, cb);
      else return listFH3Templates('apps', appHeaders, appFields, cb);
    } else if (action === 'projects') {
      if (templateId) return getTemplate('projects', templateId, cb);
      else return listFH3Templates('projects', projectHeaders, projectFields, cb);
    } else if (action === 'services') {
      // Note: services are known as 'connectors' internally in millicore
      if (templateId) return getTemplate('connectors', templateId, cb);
      else return listFH3Templates('connectors', serviceHeaders, serviceFields, cb);
    } else if (action === 'forms') {
      if(templateId) return getAF3Template('form', templateId, 'forms', cb);
      else return listAF3Templates('form', 'forms', formHeaders, formFields, cb);
    } else if (action === 'themes') {
      if(templateId) return getAF3Template('theme', templateId, 'themes', cb);
      else return listAF3Templates('theme', 'themes', themeHeaders, themeFields, cb);
    } else {
...
function listFH3Templates(type, headers, fields, cb) {
  var endp = 'box/api/templates/' + type;
  listTemplates(type, endp, null, headers, fields, cb)
}

function listAF3Templates(type, responseField, headers, fields, cb) {
  var endp = '/api/v2/forms/templates/'+type+'/list';
  listTemplates(type, endp, responseField, headers, fields, cb)
}

function listTemplates(type, endpointToCall, responseField, headers, fields, cb) {
    common.doGetApiCall(fhreq.getFeedHenryUrl(), endpointToCall, "Error reading Templates: ", function(err, temps){
      if (err) return cb(err);
      var list;

      if (responseField) {
        list = temps[responseField];
      } else {
        list = temps;
      }

      var colWidths = _.map(fields, function(f) {
          return common.maxField(list, f) + 2;
      });
...
// API not there yet - currently being developed.. In the meatime we get all the templates and pluck out the requested template
/*
 function getTemplate(type, templateId, cb) {
 common.doGetApiCall(fhreq.getFeedHenryUrl(), "box/api/templates/" + type + '/' + templateId, "Error reading Template: " + templateId, cb);
 };
 */

function getTemplate(type, templateId, cb) {
    common.doGetApiCall(fhreq.getFeedHenryUrl(), "box/api/templates/" + type, "Error reading Templates: ", function(err, temps){
        if (err) return cb(err);
...
function getAF3Template(type, templateId, key, cb){
    common.doGetApiCall(fhreq.getFeedHenryUrl(), '/api/v2/forms/templates/' +type+ '/list', "Error reading Templates: ", function(err, temps){
        if (err) return cb(err);
        temps = temps[key];
        var template = _.findWhere(temps, {_id: templateId});
        return cb(null, template);
    });
}
...
templates.completion = function (opts, cb) {
    ...
    if (argv.length === 2) {
        var cmds = ["apps", "projects", "services", "themes", "forms"];
        return cb(null, cmds);
    }
};
    ...
    requester.get(options, function(res) {
        ...
        res.on('end', function() {
            ...
            unzip.on('exit', function (code) {
                ...
                return cb(undefined, "Template copied to local file system");
            });
        });
