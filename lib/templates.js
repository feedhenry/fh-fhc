...
function templates (args, cb) {
  ngui([], function(err, isNGUI) {
    ...
    if (args.length !== 1) return cb(templates.usage_ngui);

    var action = args[0];
    if (action === 'apps') {
      var appHeaders = ['Id', 'Name', 'Type', 'Category', 'Repo'];
      var appFields = ['id', 'name', 'type', 'category', 'repoUrl'];
      return doTemplates('apps', 'Appp', appHeaders, appFields, cb);
    } else if (action === 'projects') {
      var projectHeaders = ['Id', 'Name', 'Type', 'Category'];
      var projectFields = ['id', 'name', 'type', 'category'];
      return doTemplates('projects', 'Projects', projectHeaders, projectFields, cb);
    } else {
...
function doTemplates(url, type, headers, fields, cb) {
  common.doGetApiCall(fhreq.getFeedHenryUrl(), "box/api/templates/" + url, "Error reading " + type + " Templates: ", function(err, temps){
    if (err) return cb(err);
