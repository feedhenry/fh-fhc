...
function templates (args, cb) {
    ngui([], function(err, isNGUI) {
        ...
        if (action === 'apps') {
            ...
            else return listTemplates('apps', appHeaders, appFields, cb);
        } else if (action === 'projects') {
            if (templateId) return getTemplate('projects', templateId, cb);
            else return listTemplates('projects', projectHeaders, projectFields, cb);
            ...
            else return listTemplates('connectors', serviceHeaders, serviceFields, cb);
        } else if (action === 'forms') {
            if(templateId) return getTemplate('forms', templateId, cb);
            else return listTemplates('forms', formHeaders, formFields, cb);
        } else if (action === 'themes') {
            if(templateId) return getTemplate('theme', templateId, cb);
            else return listTemplates('themes', themeHeaders, themeFields, cb);
        } else {
...
function listForms(headers, fields, cb) {
    common.doGetApiCall(fhreq.getFeedHenryUrl(), "/api/v2/forms/templates/form/list", "Error reading Templates: ", function(err, temps){
        if (err) return cb(err);
        temps = temps.forms;
...
function listThemes(headers, fields, cb) {
    common.doGetApiCall(fhreq.getFeedHenryUrl(), "/api/v2/forms/templates/theme/list", "Error reading Templates: ", function(err, temps){
        if (err) return cb(err);
        temps = temps.themes;
...
function listTemplates(type, headers, fields, cb) {
    ...
    switch (type){
        case 'forms' :
            endpointToCall = '/api/v2/forms/templates/form/list';
            break;
        case 'themes' :
            endpointToCall = '/api/v2/forms/templates/theme/list';
            break;
        default :
            endpointToCall = 'box/api/templates/' + type;
    }
