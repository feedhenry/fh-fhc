...
var api = module.exports = {
  ...
  waitForJob: function(options, cacheKey, start, cb, progressCb) {
    ...
    setTimeout(function() {
      ...
      fhreq.GET({
        ...
        else if (data[0] && data[0].status && data[0].status === 'pending') {
          start++;

          if(progressCb && data[0].logs && data[0].logs.length) {
            progressCb(null, data[0]);
          }

          api.waitForJob(options, cacheKey, start++, cb, progressCb);
        }
        else if(data[0] && data[0].status && data[0].status === 'error') {
          cb(data, null);
        } else {
  ...
  waitFor: function(options, key, cb, progressCb) {
    api.waitForJob(options, key, 0, cb, progressCb);
  },

  waitForMultiple: function(options, keys, cb) {
    async.map(keys, api.waitFor, cb);
  }
};
