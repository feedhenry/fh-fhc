...
function streamLog (appId, logName, outputFile, target, cb) {  
  ...
  common.getAppNameUrl(appId, target, function(err, appName, appUrl) {
    ...
    // TODO - check if no appName/appUrl found
    var url = appUrl.replace(appName, 'api');
    var fullUrl = url + "/streamlog/" + appName + "/" + logName;
    console.log("Streaming log to file: " + outputFile);

    //
    // Note: although we would like to stream directly to stdout here,
    // there is a bug in Request preventing us:
    // https://github.com/mikeal/request/issues/156
    // We could work around this by not using Request, but for now,
    // we stream to file.
    // request(fullUrl).pipe(process.stdout);
    //
    var str = fs.createWriteStream(outputFile);
    request(fullUrl).pipe(str);

    str.on("end", function(){      
      return cb(undefined, 'File downloaded ok..');
    });
