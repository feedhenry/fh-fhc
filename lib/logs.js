...
function logs(args, cb) { 
  ...
  if (action === "get") {
    ...
    var offset = args[3];
    var logName = args[4];
    return tailLog(appId, last, offset, logName, target, cb);
  } else if (action === "list") {
    if (!args[1]) return cb(logs.usage);
    var appId = fhc.appId(args[1]);
...
function logChunk(appName, last, offset, logName, cb) {
  var getLast = false;
  if( "undefined" == typeof(offset) || offset < 0 ) {
    last = last || 100;
    getLast = true;
  }

  common.doApiStream(fhreq.getFeedHenryUrl(), "box/srv/1.1/ide/" + fhc.target + "/app/logchunk", payload, "", function(err, response) {
...
function logTail(stream, appName, last, offset, logName, cb) {
  logChunk(appName, last, offset, logName, function(err, res) {
    if(err) return cb(err);

    var lastOffset = res.offset;
    if( "undefined" == typeof(lastOffset)) {
      return cb("offset undefined");
    }

    if( res.data.length > 0 ) {
      stream.write(res.data.join("\n") + "\n");
    }

    // Check for new logs every 1 second
    setTimeout(function() {
      logTail(stream, dyno, appName, undefined, lastOffset, logName, function(err, res) {
