...
function doSubmitFile(submissionId, fieldId, fileId, file, cb) {
  ...
  fs.readFile(file, function(err, data) {
    if (err) return cb(err);
    var url = "api/v2/forms/submission/" + submissionId + '/' + fieldId + '/' + fileId + '/submitData';
    fhreq.POST(fhreq.getFeedHenryUrl(), url, sub, function (err, remoteData, raw, res) {
      if (err) return cb(err);
      if (res.statusCode !== 200) return cb('Unexpected response code: ' + res.statusCode + ' - ' + raw);
...
function doExport(args, cb) {
  ...
  getAppFormIds(args, function(err, appId, formId, file) {
    ...
    var r = request.post({url: url, json:payload, headers: headers}).pipe(f);
...
function sampleValue(field, num) {
  ...
  if (field.type === 'dateTime') {
    if (field.fieldOptions.definition.dateTimeUnit === 'time') return '11:11:11';
    else return new Date(Date.now()).toUTCString();
  }
  ...
  if (field.type === 'location') {
    if (field.fieldOptions.definition.locationUnit === 'northEast') {
  ...
  if (field.type === 'file' || field.type === 'photo' || field.type === 'signature'){
    var f = {
      ...
      "fileSize" : field.fieldOptions.definition.file_size || 42,
  ...
  if (field.type === 'checkboxes') {
    var min = field.fieldOptions.validation.min;
    var sel = {
      selections: []
    };
    for (var i=0; i<min; i++) {
      var box = field.fieldOptions.definition.checkboxChoices[i];
      sel.selections.push(Object.keys(box)[0]);
    }
    return sel;
  }
