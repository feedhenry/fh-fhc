...
function act (args, cb) {
  ...
  read([appId], function (err, res) {
    ...
    if (nodejs) {
      ...
      if ("live" === target) {
        cb("Live environment not available for Rhino Apps");
      } else {
...
function doAct(appEnv, appId, funct, data, cb) {  
  log.silly(appEnv, "doAct appEnv");
  log.silly(appId, "doAct appId");
  log.silly(funct, "doAct funct");
  log.silly(data, "doAct data");
  common.getAppNameUrl(appId, appEnv, function(err, appName, appUrl) {
    if(err) return cb(err);
    if (!appUrl.match(/\/$/)) appUrl = appUrl + '/';
    log.silly(appName, "doAct appName");
    log.silly(appUrl, "doAct appUrl");
