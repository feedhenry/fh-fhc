...
var VALID_USER_KEY_ACTIONS = ["create", "revoke", "list", "target"];
var VALID_APP_KEY_ACTIONS = ["create", "revoke", "list"];
var API_URL = "box/srv/1.1/ide/" + fhc.domain + "/api";
var ENDPOINTS = {
  ...
  doKeys("list", data, function(err, res){
    if(err){
      ...
      if(ini.get('table') === true && r) {
        keys.table = common.createListTable({"label": 16, "key": 24, "revoked": 24, "revokedBy": 24}, r);
      }
      cb(null, r);
    }
  });
...
function createKeys(args, cb){
  ...
  if(typeof key_label === "undefined"){
    return unknown("Missong key label.", cb);
  }
  data["id"] = key_label;
...
function revokeKeys(args, cb){
  ...
  if(typeof key_val === "undefined"){
    return unknown("Miss key value.", cb);
  }
  data["key"] = key_val;
