...
function createKeys(args, cb){
  ...
  doKeys("create", data, function(err, res){
    if(err){
      ...
      cb(undefined, formatKey(res.apiKey));
    }
  });
}
...
function targetKeys(args, cb){
  ...
  if(typeof key_val === "undefined"){
    ...
    if(current_key == true || current_key == null || current_key == undefined){
      current_key = undefined;
    }
    return cb(undefined, current_key);
  } else {
    set([USER_API_KEY, key_val], function(r){
      return cb(undefined, key_val);
    });
...
function doKeys(action, data, cb){
  var url = ENDPOINTS[action].replace('<domain>', fhc.target);
