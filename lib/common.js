...
exports.createTableForNotifications = function(notifications) {
  var maxId=24, maxTS = 15, maxType = 18, maxMessage = 25;
  ...
  var table = new Table({
    head:['Guid', 'When', 'Notification Type', 'Detail'],
    colWidths: [maxId+2, maxTS+2, maxType+2, maxMessage+2],
    style: exports.style()
  });

  for(var p=0;p<notifications.length;p++){
    var not = notifications[p].fields;
    table.push([ notifications[p].guid, moment(not.sysCreated, 'YYYY-MM-DD h:mm:ss:SSS').fromNow(), not.eventType, not.message]);
  }
  return table;
}
