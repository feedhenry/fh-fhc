...
// TODO: combine 2 functions below or remove 1 of them and point to other
...
// poll remote job, with update callback for cachekey logs
exports.waitForJobWithUpdate = function(cacheKey, start, updateCb, cb) {
  setTimeout(function() {
    log.silly("polling for cacheKey: " + cacheKey + " start: " + start + " to complete", "Polling");

    var uri = 'box/srv/1.1/dat/log/read';
    var params = {
      cacheKeys: [{
        cacheKey: cacheKey,
        start: start
    ...
    fhreq.POST(fhreq.getFeedHenryUrl(), uri, params, function (err, remoteData, raw, response){
      if (err) return cb(err);

      if (remoteData[0] && remoteData[0].log) {
        updateCb(remoteData[0].log);
      }

      if (remoteData[0] && remoteData[0].status !== undefined && remoteData[0].status === 'pending') {
        log.silly(remoteData[0].log, "waitforjobwithupdate");
        start += remoteData[0].log.length;
        return exports.waitForJobWithUpdate(cacheKey, start, updateCb, cb);
      } else {
        if(remoteData[0] && remoteData[0].status && remoteData[0].status === 'error') {
          return cb(remoteData);
        }else {
...
exports.createTableForCredentials = function(credentials) {
  ...
  _.each(credentials, function(bundle){
    table.push([bundle.id, bundle.name, bundle.platform, bundle.type]);
  }, this);
