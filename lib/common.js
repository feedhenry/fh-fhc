...
exports.getAppNameUrl = function(appId, env, cb){
  exports.doApiCall(fhreq.getFeedHenryUrl(), "box/srv/1.1/ide/" + fhc.domain + "/app/hosts", {payload: {guid: appId}}, "", function(err, data){
    if(err) return cb(err);
    log.verbose(data, "hosts");
...
// common read app statistics
function readAppStats(appId, deployTarget, statsType, numResults, cb) {
  var payload = {payload:{guid: appId, deploytarget: deployTarget, statstype: statsType, count: numResults}};
  exports.doApiCall(fhreq.getFeedHenryUrl(), "box/srv/1.1/ide/" + fhc.domain + "/app/stats", payload, "Error reading app statistics: ", cb);
};
...
exports.readAppStats = readAppStats;
