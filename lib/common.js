...
function getFeedHenryDomain() {
  var url = require('url');
  var domain;
  try {
    var uri = fhreq.getFeedHenryUrl();
    var host = url.parse(uri).hostname;
    var parts = host.split('.');
    log.silly(parts, 'FeedHenry Domain Parts');
    domain = parts[0];    
  } catch (x) {
    log.warn("Unexpected error in getFeedHenryDomain: " + x);    
    domain = 'apps';
  }
  return domain;
};
...
exports.getFeedHenryDomain = getFeedHenryDomain;
