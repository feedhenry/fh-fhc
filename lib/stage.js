...
stage.usage = "\nfhc stage <appId>"
            ...
            + "\nfhc stage <appId> [approver] [comment] runtime=<node06|node08|node010>> --env=<environment> "
            +"\nfhc stage <appId> [number_of_instances] [approver] [comment] runtime=<node06|node08|node010> --env=<environment>"
            + "\nStage is made to dev environment by default, specify another using the '--env=<environment>' flag"
...
function stage (args, cb) {
  ...
  var target = ini.has('env') ? ini.get('env') : 'development';
	var runtime;
  var numappinstances;
  var approver;
  var comment;
...
function doStage(app, target, clean, numappinstances, approver, comment, runtime, registry, cb) {
  ...
  startStage(app, type, clean, numappinstances, approver, comment, runtime, registry, function(err, results){
    if(err){
...
function startStage(app, type, clean, numappinstances, approver, comment, runtime, registry, cb){
  ...
  if (registry && registry !== '') payload.payload.registry = registry;
  if(numappinstances) {
