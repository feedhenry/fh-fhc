...
stage.usage = "\nfhc stage <appId>"
						+ "\nfhc stage <appId> runtime=<node06|node08|node010>"
            + "\nfhc stage <appId> [approver] [comment] runtime=<node06|node08|node010>> --live "
            +"\nfhc stage <appId> [number_of_instances] [approver] [comment] runtime=<node06|node08|node010> --live"
  ...
  function processArg(arg) {
    ...
    if(Number(arg)) {
      ...
      clean = true;
	    ...
	    var bits = arg.split("=");
	    if(bits.length != 2 || bits[1] == '')return cb("runtime needs to be set as runtime=node08");
