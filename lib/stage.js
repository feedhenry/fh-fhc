...
stage.usage = "\nfhc stage <appId>"
						+ "\nfhc stage <appId> runtime=node06"
            + "\nfhc stage <appId> [approver] [comment] --live runtime=node06"
            +"\nfhc stage <appId> [number_of_instances] [approver] [comment] runtime=node06 --live"
...
function stage (args, cb) {
  ...
  function processArg(arg) {
    ...
    if(Number(arg)) {
      ...
      clean = true;
    }else if (arg.indexOf("runtime") != -1){
	    var bits = arg.split("=");
	    if(bits.length < 2)return cb("runtime needs to be set as runtime=node08");
  ...
  doStage(fhc.appId(args[0]), target, clean, numappinstances, approver, comment, runtime, cb);
};
...
// do our staging..
function doStage(app, target, clean, numappinstances, approver, comment, runtime, cb) {
  var type = 'stage';
  ...
  startStage(app, type, clean, numappinstances, approver, comment, runtime, function(err, results){
    if(err){
        ...
        prompt("Please enter approver:", "", false, function(err, val){
          ...
          if(val && val.length > 0){
            ...
            prompt("Please enter comment:", "", false, function(err, cval){
              ...
              startStage(app, type, clean, numappinstances, approver, comment, runtime, cb);
            });
          } else {
...
function startStage(app, type, clean, numappinstances, approver, comment,runtime,cb){
