...
nativecfg.usage  = "usage: fhc native config=<apple | android> app=<appid | alias> [dir=<PATH TO DIR TO WRIE FILE (CLI only)>]\n";
var fhc     = require("./fhc"),
  alias     = require("./alias"),
...
function nativecfg(args, cb) {
  ...
  validateArgs(parsed,function (err) {
    ...
    read([parsed.app], function (err, data) {
      if (err)return cb(err);
