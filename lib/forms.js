...
forms.usage = "fhc forms [list]\n" +
              ...
              "fhc forms groups update <group-id> <group-name> <user-id>* <form-id>*    # user-id and form-id should be a comma-seperated list of values\n" +
              "fhc forms groups create <group-name> <user-id>* <form-id>*               # user-id and form-id should be a comma-seperated list of values\n" +
...
function forms (args, cb) {
  ...
  if (action === 'groups') {
    ...
    if (act === 'update') {
      if (args.length != 6) return cb(forms.usage);
      ...
      return doFormsGroupUpdate(groupId, groupName, userIds, formIds, cb);
    }
    if (act === 'create') {
      if (args.length != 5) return cb(forms.usage);

      var groupName = args[2];
      var userIds = args[3].split(',');
      var formIds = args[4].split(',');

      return doFormsGroupCreate(groupName, userIds, formIds, cb);
    }
    if (act === 'delete') {
      var groupId = args[2];
      if (!groupId) return cb(forms.usage);
...
function doFormsGroupUpdate(groupId, groupName, userIds, formIds, cb) {
  fhreq.POST(fhreq.getFeedHenryUrl(), URL_PREFIX_API_FORMS_GROUP + "/" + groupId, {name: groupName, users: userIds, forms: formIds}, function (err, remoteData, raw, res) {
    if (err) return cb(err);
    if (res.statusCode !== 200) return cb('Unexpected response code: ' + res.statusCode + ' - ' + raw);
...
function doFormsGroupCreate(groupName, userIds, formIds, cb) {
  fhreq.POST(fhreq.getFeedHenryUrl(), URL_PREFIX_API_FORMS_GROUP + "", {name: groupName, users: userIds, forms: formIds}, function (err, remoteData, raw, res) {
    if (err) return cb(err);
    if (res.statusCode !== 200) return cb('Unexpected response code: ' + res.statusCode + ' - ' + raw);
