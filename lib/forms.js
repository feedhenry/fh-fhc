...
function forms (args, cb) {
    ...
    if (action === 'apps') {
        ...
        if (act === 'update') {
            ...
            if (!projectId) return cb(forms.usage);

            var formIds = ("" === args[3]) ? [] : _.rest(args, 3);
            //if (formIds.length == 0) return cb(forms.usage);
            return doAppFormsUpdate(projectId, formIds, cb);
        }
        if (act === 'create') {
            var appTitle = args[2];
...
function doAppFormsUpdate(projectId, formIds, cb) {
    fhreq.POST(fhreq.getFeedHenryUrl(), URL_PREFIX_API_FORMS_APPS + "/" + projectId, {forms: formIds}, function (err, remoteData, raw, res) {
        if (err) return cb(err);
        if (res.statusCode !== 200) return cb('Unexpected response code: ' + res.statusCode + ' - ' + raw);
