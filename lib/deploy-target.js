...
deployTargets.listAvailable = listAvailableDeployTargets;
deployTargets.getTargetForApp = getDeployTargetForApp;
deployTargets.setTargetForApp = setDeployTargetForApp;

deployTargets.usage = "\nfhc deploy-target create name=<the name of the deploy target> platform=<feedhenry/cloudfoundry/stackato> env=<dev/live/all> conf_url=<deploy url> conf_username=<user for the deploy> conf_password=<user password for the deploy> ..."
                    + "\nfhc deploy-target update id=<id of the deploy target> name=<the name of the deploy target> platform=<feedhenry/cloudfoundry/stackato> env=<dev/live/all> conf_url=<deploy url> conf_username=<user for the deploy> conf_password=<user password for the deploy> ... "
                    ...
                    + "\nfhc deploy-target listforapp <appid> <platform> <dev/live>"
...
function deployTargets(args, cb){
  ...
  if(action === "list"){
    ...
    if(args.length < 2) return cb("Invalid arguments for 'availableforapp':" + deployTargets.usage);
