...
function list (appId, cb) {
  ...
  millicore.widgForAppId(appId, function (err, widgId) {
    ...
    common.doApiCall(fhreq.getFeedHenryUrl(), "box/srv/1.1/ide/" + fhc.target + "/file/list", payload, "Error listing file: ", function(err, data){
      var filez = {};
...
function read (fileId, cb) {  
  ...
  common.doApiCall(fhreq.getFeedHenryUrl(), "box/srv/1.1/ide/" + fhc.target + "/file/read", payload, "Error reading file: ", cb);
};

function update (appId, fileId, file, cb){
  if (!fileId) return cb("No fileId specified! Usage:\n" + files.usage);
  if (!file) return cb("No file specified! Usage:\n" + files.usage);
  var apiURL = "box/srv/1.1/ide/" + fhc.target + "/file/update";
...
function create (appId, path, name, fileType, cb){
  ...
  millicore.widgForAppId(appId, function (err, widgId) {
    ...
    log.silly(widgId, "widgId");
      ...
      common.doApiCall(fhreq.getFeedHenryUrl(),"box/srv/1.1/ide/" + fhc.target + "/file/create", payload, "Error creating file: ", cb);
  });
};
...
function deleteFile (appId, fileId, path, name, type, cb){
  ...
  millicore.widgForAppId(appId, function (err, widgId) {
    ...
    log.silly(widgId, "widgId");
      ...
      common.doApiCall(fhreq.getFeedHenryUrl(),"box/srv/1.1/ide/" + fhc.target + "/file/delete", payload, "Error deleting file: ", cb);
  });
};
