...
build.usage = "\nfhc build <command> <parameters>"
  + "\nwhere <command> is one of: make, start, status (may be missing, then defaults to make)"
  + "\n"
  + "\napp=<appId> destination=<destination> version=<version> config=<config> keypass=<private-key-password> certpass=<certificate-password> download=<true|false> provisioning=<path-to-provisioning-profile>"
...
function build (args, cb) {
  ...
  argObj.callback = function(data){ cb(null, data); };
  log(Object.keys(argObj));
  commands[argObj.command](argObj);mariapabis@gmail.com
};
...
commands.status = function(args) {
  //TODO Consider making it standalone command.
  common.isJobDone(args.cacheKey, args.start, function(err, response) {
    log.warn("Done? " + response.done);
    log.warn("resp: " + JSON.stringify(response));
    args.callback(err, response.data);
  })
};
commands.make = function(args) {
  startBuild(args, function(keys) {
