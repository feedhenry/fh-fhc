...
create.usage_ngui = "fhc create <project-id> <app-title> [<type>]";
...
function create (args, cb) {
  ...
  ngui([], function(err, isNGUI) {
    ...
    if (isNGUI) {
      var project = args[0];
      var title = args[1];
      var type = args[2] || "cloud_nodejs";
      return doCreateNgui(project, title, type, cb);
    } else {
      var title = args[0];
      var repo = args[1];
      var branch = args[2];
...
function doCreateNgui(projectId, title, type, cb) {
  var payload = {
    "title": title,
    "connections":[],
    "template": {
