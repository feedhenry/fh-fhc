...
function completion (args, cb) {
  if (process.platform === "win32") {
    var e = new Error("fhc completion not supported on windows")
    e.code = "ENOTSUP";
  ...
  // try to find the fhc command.
  // it's the first thing after all the configs.
  // take a little shortcut and use fhc's arg parsing logic.
  ...
  Object.keys(parsed).forEach(function (k) {
    fhc.config.set(k, parsed[k]);
  });

  // at this point, if words[1] is some kind of fhc command,
  // then complete on it.
  // otherwise, do nothing
  cmd = fhc.commands[cmd];
...
function dumpScript (cb) {
  ...
  fs.readFile(p, "utf8", function (er, d) {
    ...
    process.stdout.on("error", function (er) {
      ...
      // errors, so casually.  But, without this, `. <(fhc completion)`
...
// complete against the fhc commands
function cmdCompl (opts, cb) {
  return cb(null, fhc.fullList);
}
